...
export function CardSkeleton() {
  return (
    <div className="glass-card no-hover" style={{ padding: "22px 24px" }}>
      <Skeleton h={11} w="50%" r={6} style={{ marginBottom: 12 }} />
      <Skeleton h={28} w="65%" r={8} style={{ marginBottom: 16 }} />
...
export function ErrorBox({ message, onRetry }) {
  return (
    <div className="glass-card no-hover" style={{ padding: "40px 24px", textAlign: "center" }}>
  ...
  return (
    <div className={`glass-card card-in ${delayClass}`} style={{ padding: "22px 24px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 11, color: "var(--text-muted)", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600, marginBottom: 8 }}>
            {label}
          </div>
          <div
            className="mono grad-text"
            style={{ fontSize: 26, fontWeight: 700, letterSpacing: "-0.03em", background: `linear-gradient(135deg,${color},${color}99)` }}
          >
            {value}
          </div>
        </div>
