    ...
    <>
      ...
      <div className="glass-card no-hover" style={{ padding:"16px 20px", marginBottom:16, display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ position:"relative", flex:"1 1 180px", minWidth:160 }}>
          ...
          <input className="fin-input" placeholder="Search transactions…" value={search}
        ...
        <div style={{ position:"relative" }}>
          <select className="fin-select" value={filterType} onChange={e => { setType(e.target.value); setPage(1); }} style={{ width:"auto" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          ...
          {isAdmin && (
            <button className="btn btn-primary" onClick={() => { setEditing(null); setShowForm(v => !v); }}>
              {showForm ? "✕ Close" : "+ Add"}
            </button>
          )}
        </div>
      ...
      {txnQ.error ? (
        ...
        <div className="glass-card no-hover" style={{ overflow:"hidden" }}>
            ...
            {txnQ.loading ? (
              ...
              <table className="fin-table">
                  ...
                  <tr>
                    {SORT_FIELDS.map(f => (
                      <th key={f.key} onClick={() => handleSort(f.key)}>
                        {f.label}<span style={{ opacity:0.45 }}>{sortIcon(f.key)}</span>
                      </th>
                    ))}
                ...
                <tbody>
                  {transactions.map(t => (
                    <tr key={t.id}>
                      <td style={{ color:"var(--text-secondary)", whiteSpace:"nowrap" }}>{fmtDate(t.txn_date)}</td>
                      <td>
                        ...
                        <td>
                          <div style={{ display:"flex", gap:8 }}>
                            <button className="btn btn-ghost" style={{ padding:"4px 12px", fontSize:11 }}
                              onClick={() => { setEditing(t); setShowForm(true); window.scrollTo({top:0,behavior:"smooth"}); }}>
                              Edit
                            </button>
          ...
          {/* Pagination */}
          {pagination.pages > 1 && (
            <div style={{ padding:"14px 20px", borderTop:"1px solid var(--glass-border)", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
          ...
          {!txnQ.loading && transactions.length > 0 && pagination.pages <= 1 && (
